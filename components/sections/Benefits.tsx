        ...
        <Reveal>
          ...
          <div className="bg-line border-line grid grid-cols-2 gap-px overflow-hidden rounded-[14px] border shadow-[0_10px_40px_-24px_rgba(31,33,25,0.35)] lg:grid-cols-4">
            {benefits.map(({ Icon, title, subtitle }) => (
              // group на ячейке, увеличение на вложенной паре: подложка
              // остаётся неподвижной, но наведение ловится по всей ячейке
              <div
                key={title}
                className="group bg-card flex items-center px-5 py-6 sm:px-7 lg:py-7"
              >
                <div className="hover-zoom-item flex items-center gap-3.5 sm:gap-4">
                  <Icon className="text-accent h-[26px] w-[26px] shrink-0" />
